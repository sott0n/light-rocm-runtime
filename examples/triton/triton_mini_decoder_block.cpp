...
#ifndef LRRT_TRITON_MINI_DECODER_RMSNORM_MANIFEST
#define LRRT_TRITON_MINI_DECODER_RMSNORM_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_MATVEC_MANIFEST
#define LRRT_TRITON_MINI_DECODER_MATVEC_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_ATTENTION_SCORE_MANIFEST
#define LRRT_TRITON_MINI_DECODER_ATTENTION_SCORE_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_CAUSAL_SOFTMAX_MANIFEST
#define LRRT_TRITON_MINI_DECODER_CAUSAL_SOFTMAX_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_VALUE_AGGREGATION_MANIFEST
#define LRRT_TRITON_MINI_DECODER_VALUE_AGGREGATION_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_VECTOR_ADD_MANIFEST
#define LRRT_TRITON_MINI_DECODER_VECTOR_ADD_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_KV_CACHE_MANIFEST
#define LRRT_TRITON_MINI_DECODER_KV_CACHE_MANIFEST "manifest.json"
#endif

#ifndef LRRT_TRITON_MINI_DECODER_ROPE_MANIFEST
#define LRRT_TRITON_MINI_DECODER_ROPE_MANIFEST "manifest.json"
#endif
...
static uint32_t select_hidden_block(uint32_t hidden) {
  if (hidden <= 1024) {
    return 1024;
  }
  if (hidden <= 2048) {
...
class MiniDecoderBlockExecutor {
public:
  MiniDecoderBlockExecutor(lrrt::Device &device, uint32_t keys, uint32_t hidden,
                           uint32_t head_dim)
    ...
    bundles_.add("rmsnorm", LRRT_TRITON_MINI_DECODER_RMSNORM_MANIFEST,
                 "rmsnorm_fp32_" + std::to_string(select_hidden_block(hidden)));
    bundles_.add("q_projection", LRRT_TRITON_MINI_DECODER_MATVEC_MANIFEST,
                 "matvec_fp32_" + std::to_string(select_hidden_block(hidden)));
    bundles_.add("k_projection", LRRT_TRITON_MINI_DECODER_MATVEC_MANIFEST,
                 "matvec_fp32_" + std::to_string(select_hidden_block(hidden)));
    bundles_.add("v_projection", LRRT_TRITON_MINI_DECODER_MATVEC_MANIFEST,
                 "matvec_fp32_" + std::to_string(select_hidden_block(hidden)));
    bundles_.add("out_projection", LRRT_TRITON_MINI_DECODER_MATVEC_MANIFEST,
                 "matvec_fp32_" +
                     std::to_string(select_hidden_block(head_dim)));
    bundles_.add("score", LRRT_TRITON_MINI_DECODER_ATTENTION_SCORE_MANIFEST,
                 "attention_score_fp32_" +
                     std::to_string(select_head_block(head_dim)));
    bundles_.add("softmax", LRRT_TRITON_MINI_DECODER_CAUSAL_SOFTMAX_MANIFEST,
                 "causal_softmax_fp32_" +
                     std::to_string(select_keys_block(keys)));
    bundles_.add(
        "aggregation", LRRT_TRITON_MINI_DECODER_VALUE_AGGREGATION_MANIFEST,
        "value_aggregation_fp32_" + std::to_string(select_keys_block(keys)));
    bundles_.add("residual_add", LRRT_TRITON_MINI_DECODER_VECTOR_ADD_MANIFEST,
                 "vector_add");
    bundles_.add("kv_update", LRRT_TRITON_MINI_DECODER_KV_CACHE_MANIFEST,
                 "kv_cache_update_fp32_" +
                     std::to_string(select_head_block(head_dim)));
    bundles_.add("rope", LRRT_TRITON_MINI_DECODER_ROPE_MANIFEST,
    ...
    buffers_.allocate<float>("norm_states", keys * hidden);
  ...
  void copy_inputs(const std::vector<float> &hidden_states,
    ...
    if (hidden_states.size() != keys_ * hidden_ ||
      ...
      throw std::runtime_error("mini decoder block input shape mismatch");
    }
    ...
    buffers_.copy_to("norm_states", hidden_cache_zero);
  ...
  void run(uint32_t valid_keys) {
    if (valid_keys == 0 || valid_keys > keys_) {
      throw std::runtime_error("mini decoder block valid_keys is out of range");
    }

    const uint32_t half = head_dim_ / 2;
    const uint32_t query_position = valid_keys - 1;
    tex::launch(queue_, bundles_.get("rmsnorm"), keys_,
                {
                    tex::arg("x", buffers_.ptr<float>("hidden_states")),
                    tex::arg("weight", buffers_.ptr<float>("norm_weight")),
                    tex::arg("out", buffers_.ptr<float>("norm_states")),
                    tex::arg("eps", 1.0e-5f),
                    tex::arg("rows", (int32_t)keys_),
                    tex::arg("hidden", (int32_t)hidden_),
                });

    tex::launch(queue_, bundles_.get("q_projection"), head_dim_,
                {
                    tex::arg("x", buffers_.ptr<float>(
                                      "norm_states", query_position * hidden_)),
    ...
    for (uint32_t position = 0; position < valid_keys; ++position) {
      tex::launch(queue_, bundles_.get("k_projection"), head_dim_,
                  {
                      tex::arg("x", buffers_.ptr<float>("norm_states",
                                                        position * hidden_)),
  ...
  void copy_output(std::vector<float> &out) const {
    if (out.size() != hidden_) {
      throw std::runtime_error("mini decoder block output shape mismatch");
    }
...
static void run_case(lrrt::Device &device, uint32_t keys, uint32_t hidden,
  ...
  std::vector<float> norm_states(keys * hidden, 0.0f);
  ...
  MiniDecoderBlockExecutor executor(device, keys, hidden, head_dim);
  executor.copy_inputs(hidden_states, norm_weight, q_weight, k_weight, v_weight,
                       out_weight, cos, sin);
  executor.run(valid_keys);
  executor.synchronize();
  executor.copy_output(out);

  reference_rmsnorm(hidden_states, norm_weight, keys, hidden, norm_states);

  const uint32_t query_position = valid_keys - 1;
  std::vector<float> q(head_dim);
  std::vector<float> k(keys * head_dim, 0.0f);
  std::vector<float> v(keys * head_dim, 0.0f);
  reference_projection(norm_states, q_weight, hidden, query_position, q);
  for (uint32_t row = 0; row < valid_keys; ++row) {
    std::vector<float> k_row(head_dim);
    std::vector<float> v_row(head_dim);
    reference_projection(norm_states, k_weight, hidden, row, k_row);
    reference_projection(norm_states, v_weight, hidden, row, v_row);
  ...
  if (max_diff > 0.01f) {
    fprintf(stderr,
            "triton_mini_decoder_block keys=%u hidden=%u head_dim=%u "
            "valid_keys=%u mismatch at %u: actual=%f expected=%f diff=%f\n",
            keys, hidden, head_dim, valid_keys, max_index, max_actual,
            max_expected, max_diff);
    throw std::runtime_error("triton_mini_decoder_block result mismatch");
  }
}
...
int main(void) {
  try {
    ...
    run_case(device, 64, 1024, 128, 33);

    printf("triton_mini_decoder_block: ok\n");
    return 0;
