...
| Qwen decode-step VMFB export | ✅ | Exposes the same RoPE + KV cache + FFN entry point through a stable `qwen_decode_step` export so Qwen-oriented runners do not depend on the internal mini-decoder name. | `lrrt_iree_qwen_decode_step_run_module_vmfb_smoke` |
...
| Two-token Qwen decode E2E runner | ✅ | Uses `VmfbRunner` to call `qwen_decode_step` twice and passes step 1 K/V cache buffer views directly into step 2. | `lrrt_iree_qwen_decode_e2e_smoke` |
