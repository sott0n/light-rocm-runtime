...
cmake --build build-sparsewave/e2e \
  --target lrrt_sparsewave_python lrrt_sparsewave_spmm_bundle
PYTHONPATH="$PWD/build-sparsewave/e2e/executor/sparsewave/python" \
  LRRT_SPARSEWAVE_SPMM_MANIFEST="$PWD/build-sparsewave/e2e/executor/sparsewave/spmm.bundle/manifest.json" \
...
SparseWave application. SpMM is the first registered application; additional
hand-written executors can be added without changing the public API. The loaded
executor retains its HSACO and can be reused with different values and NNZ
counts. SparseWave owns graph export and compilation; the Python binding only
loads and executes compiled artifacts.
