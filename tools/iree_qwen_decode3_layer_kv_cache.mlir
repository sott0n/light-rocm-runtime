func.func @qwen_decode3_layer_kv_cache(
    %input: tensor<1x896xf32>,
    %old_key_cache: tensor<2x128xf32>,
    %old_value_cache: tensor<2x128xf32>,
    ...
    %down_weight: tensor<4864x896xf32>) -> (tensor<3x128xf32>, tensor<3x128xf32>, tensor<1x896xf32>) {
  ...
  %key_cache_empty = tensor.empty() : tensor<3x128xf32>
  %key_cache_old = tensor.insert_slice %old_key_cache into %key_cache_empty[0, 0] [2, 128] [1, 1]
      : tensor<2x128xf32> into tensor<3x128xf32>
  %key_cache = tensor.insert_slice %k into %key_cache_old[2, 0] [1, 128] [1, 1]
      : tensor<1x128xf32> into tensor<3x128xf32>

  %value_cache_empty = tensor.empty() : tensor<3x128xf32>
  %value_cache_old = tensor.insert_slice %old_value_cache into %value_cache_empty[0, 0] [2, 128] [1, 1]
      : tensor<2x128xf32> into tensor<3x128xf32>
  %value_cache = tensor.insert_slice %v into %value_cache_old[2, 0] [1, 128] [1, 1]
      : tensor<1x128xf32> into tensor<3x128xf32>
  ...
  %q1 = tensor.extract_slice %q[0, 448] [1, 448] [1, 1]
      : tensor<1x896xf32> to tensor<1x448xf32>
  %key_cache0 = tensor.extract_slice %key_cache[0, 0] [3, 64] [1, 1]
      : tensor<3x128xf32> to tensor<3x64xf32>
  %key_cache1 = tensor.extract_slice %key_cache[0, 64] [3, 64] [1, 1]
      : tensor<3x128xf32> to tensor<3x64xf32>
  %value_cache0 = tensor.extract_slice %value_cache[0, 0] [3, 64] [1, 1]
      : tensor<3x128xf32> to tensor<3x64xf32>
  %value_cache1 = tensor.extract_slice %value_cache[0, 64] [3, 64] [1, 1]
      : tensor<3x128xf32> to tensor<3x64xf32>
  ...
  %key_head0_empty = tensor.empty() : tensor<64x3xf32>
  %key_head0 = linalg.generic {
      ...
      ins(%key_cache0 : tensor<3x64xf32>)
      outs(%key_head0_empty : tensor<64x3xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<64x3xf32>

  %key_head1_empty = tensor.empty() : tensor<64x3xf32>
  %key_head1 = linalg.generic {
      ...
      ins(%key_cache1 : tensor<3x64xf32>)
      outs(%key_head1_empty : tensor<64x3xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<64x3xf32>

  %score0_empty = tensor.empty() : tensor<7x3xf32>
  %score0_init = linalg.fill ins(%zero : f32)
      outs(%score0_empty : tensor<7x3xf32>) -> tensor<7x3xf32>
  %scores0_unscaled = linalg.matmul
      ins(%q_heads0, %key_head0 : tensor<7x64xf32>, tensor<64x3xf32>)
      outs(%score0_init : tensor<7x3xf32>) -> tensor<7x3xf32>

  %score1_empty = tensor.empty() : tensor<7x3xf32>
  %score1_init = linalg.fill ins(%zero : f32)
      outs(%score1_empty : tensor<7x3xf32>) -> tensor<7x3xf32>
  %scores1_unscaled = linalg.matmul
      ins(%q_heads1, %key_head1 : tensor<7x64xf32>, tensor<64x3xf32>)
      outs(%score1_init : tensor<7x3xf32>) -> tensor<7x3xf32>

  %scaled_scores0_empty = tensor.empty() : tensor<7x3xf32>
  %scaled_scores0 = linalg.generic {
      ...
      ins(%scores0_unscaled : tensor<7x3xf32>)
      outs(%scaled_scores0_empty : tensor<7x3xf32>) {
    ^bb0(%score: f32, %out: f32):
      %scaled = arith.mulf %score, %head_scale : f32
      linalg.yield %scaled : f32
    } -> tensor<7x3xf32>

  %scaled_scores1_empty = tensor.empty() : tensor<7x3xf32>
  %scaled_scores1 = linalg.generic {
      ...
      ins(%scores1_unscaled : tensor<7x3xf32>)
      outs(%scaled_scores1_empty : tensor<7x3xf32>) {
    ^bb0(%score: f32, %out: f32):
      %scaled = arith.mulf %score, %head_scale : f32
      linalg.yield %scaled : f32
    } -> tensor<7x3xf32>
  ...
  %score_max0 = linalg.generic {
      ...
      ins(%scaled_scores0 : tensor<7x3xf32>)
  ...
  %score_max1 = linalg.generic {
      ...
      ins(%scaled_scores1 : tensor<7x3xf32>)
  ...
  %exp0_empty = tensor.empty() : tensor<7x3xf32>
  %exp_scores0 = linalg.generic {
      ...
      ins(%scaled_scores0, %score_max0 : tensor<7x3xf32>, tensor<7xf32>)
      outs(%exp0_empty : tensor<7x3xf32>) {
  ...
  %exp1_empty = tensor.empty() : tensor<7x3xf32>
  %exp_scores1 = linalg.generic {
      ...
      ins(%scaled_scores1, %score_max1 : tensor<7x3xf32>, tensor<7xf32>)
      outs(%exp1_empty : tensor<7x3xf32>) {
    ^bb0(%score: f32, %max: f32, %out: f32):
      %shifted = arith.subf %score, %max : f32
      %exp = math.exp %shifted : f32
      linalg.yield %exp : f32
  ...
  %row_sums0 = linalg.generic {
      ...
      ins(%exp_scores0 : tensor<7x3xf32>)
  ...
  %row_sums1 = linalg.generic {
      ...
      ins(%exp_scores1 : tensor<7x3xf32>)
  ...
  %prob0_empty = tensor.empty() : tensor<7x3xf32>
  %probabilities0 = linalg.generic {
      ...
      ins(%exp_scores0, %row_sums0 : tensor<7x3xf32>, tensor<7xf32>)
      outs(%prob0_empty : tensor<7x3xf32>) {
    ^bb0(%exp_score: f32, %row_sum: f32, %out: f32):
      %probability = arith.divf %exp_score, %row_sum : f32
      linalg.yield %probability : f32
    } -> tensor<7x3xf32>

  %prob1_empty = tensor.empty() : tensor<7x3xf32>
  %probabilities1 = linalg.generic {
      ...
      ins(%exp_scores1, %row_sums1 : tensor<7x3xf32>, tensor<7xf32>)
      outs(%prob1_empty : tensor<7x3xf32>) {
    ^bb0(%exp_score: f32, %row_sum: f32, %out: f32):
      %probability = arith.divf %exp_score, %row_sum : f32
      linalg.yield %probability : f32
    } -> tensor<7x3xf32>

  %value_head0_empty = tensor.empty() : tensor<3x64xf32>
  %value_head0 = linalg.generic {
      ...
      ins(%value_cache0 : tensor<3x64xf32>)
      outs(%value_head0_empty : tensor<3x64xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<3x64xf32>

  %value_head1_empty = tensor.empty() : tensor<3x64xf32>
  %value_head1 = linalg.generic {
      ...
      ins(%value_cache1 : tensor<3x64xf32>)
      outs(%value_head1_empty : tensor<3x64xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<3x64xf32>
  ...
  %context_heads0 = linalg.matmul
      ins(%probabilities0, %value_head0 : tensor<7x3xf32>, tensor<3x64xf32>)
  ...
  %context_heads1 = linalg.matmul
      ins(%probabilities1, %value_head1 : tensor<7x3xf32>, tensor<3x64xf32>)
  ...
  return %key_cache, %value_cache, %result
      : tensor<3x128xf32>, tensor<3x128xf32>, tensor<1x896xf32>
}
