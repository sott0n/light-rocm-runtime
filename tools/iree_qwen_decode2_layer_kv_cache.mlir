func.func @qwen_decode2_layer_kv_cache(
  ...
  %q1 = tensor.extract_slice %q[0, 448] [1, 448] [1, 1]
      : tensor<1x896xf32> to tensor<1x448xf32>
  %key_cache0 = tensor.extract_slice %key_cache[0, 0] [2, 64] [1, 1]
      : tensor<2x128xf32> to tensor<2x64xf32>
  %key_cache1 = tensor.extract_slice %key_cache[0, 64] [2, 64] [1, 1]
      : tensor<2x128xf32> to tensor<2x64xf32>
  %value_cache0 = tensor.extract_slice %value_cache[0, 0] [2, 64] [1, 1]
      : tensor<2x128xf32> to tensor<2x64xf32>
  %value_cache1 = tensor.extract_slice %value_cache[0, 64] [2, 64] [1, 1]
      : tensor<2x128xf32> to tensor<2x64xf32>
  ...
  %key_head0_empty = tensor.empty() : tensor<64x2xf32>
  %key_head0 = linalg.generic {
      ...
      ins(%key_cache0 : tensor<2x64xf32>)
      outs(%key_head0_empty : tensor<64x2xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<64x2xf32>

  %key_head1_empty = tensor.empty() : tensor<64x2xf32>
  %key_head1 = linalg.generic {
      ...
      ins(%key_cache1 : tensor<2x64xf32>)
      outs(%key_head1_empty : tensor<64x2xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<64x2xf32>

  %score0_empty = tensor.empty() : tensor<7x2xf32>
  %score0_init = linalg.fill ins(%zero : f32)
      outs(%score0_empty : tensor<7x2xf32>) -> tensor<7x2xf32>
  %scores0_unscaled = linalg.matmul
      ins(%q_heads0, %key_head0 : tensor<7x64xf32>, tensor<64x2xf32>)
      outs(%score0_init : tensor<7x2xf32>) -> tensor<7x2xf32>

  %score1_empty = tensor.empty() : tensor<7x2xf32>
  %score1_init = linalg.fill ins(%zero : f32)
      outs(%score1_empty : tensor<7x2xf32>) -> tensor<7x2xf32>
  %scores1_unscaled = linalg.matmul
      ins(%q_heads1, %key_head1 : tensor<7x64xf32>, tensor<64x2xf32>)
      outs(%score1_init : tensor<7x2xf32>) -> tensor<7x2xf32>

  %scaled_scores0_empty = tensor.empty() : tensor<7x2xf32>
  %scaled_scores0 = linalg.generic {
      ...
      ins(%scores0_unscaled : tensor<7x2xf32>)
      outs(%scaled_scores0_empty : tensor<7x2xf32>) {
    ^bb0(%score: f32, %out: f32):
      %scaled = arith.mulf %score, %head_scale : f32
      linalg.yield %scaled : f32
    } -> tensor<7x2xf32>

  %scaled_scores1_empty = tensor.empty() : tensor<7x2xf32>
  %scaled_scores1 = linalg.generic {
      ...
      ins(%scores1_unscaled : tensor<7x2xf32>)
      outs(%scaled_scores1_empty : tensor<7x2xf32>) {
    ^bb0(%score: f32, %out: f32):
      %scaled = arith.mulf %score, %head_scale : f32
      linalg.yield %scaled : f32
    } -> tensor<7x2xf32>
  ...
  %score_max0 = linalg.generic {
      ...
      ins(%scaled_scores0 : tensor<7x2xf32>)
  ...
  %score_max1 = linalg.generic {
      ...
      ins(%scaled_scores1 : tensor<7x2xf32>)
  ...
  %exp0_empty = tensor.empty() : tensor<7x2xf32>
  %exp_scores0 = linalg.generic {
      ...
      ins(%scaled_scores0, %score_max0 : tensor<7x2xf32>, tensor<7xf32>)
      outs(%exp0_empty : tensor<7x2xf32>) {
  ...
  %exp1_empty = tensor.empty() : tensor<7x2xf32>
  %exp_scores1 = linalg.generic {
      ...
      ins(%scaled_scores1, %score_max1 : tensor<7x2xf32>, tensor<7xf32>)
      outs(%exp1_empty : tensor<7x2xf32>) {
    ^bb0(%score: f32, %max: f32, %out: f32):
      %shifted = arith.subf %score, %max : f32
      %exp = math.exp %shifted : f32
      linalg.yield %exp : f32
  ...
  %row_sums0 = linalg.generic {
      ...
      ins(%exp_scores0 : tensor<7x2xf32>)
  ...
  %row_sums1 = linalg.generic {
      ...
      ins(%exp_scores1 : tensor<7x2xf32>)
  ...
  %prob0_empty = tensor.empty() : tensor<7x2xf32>
  %probabilities0 = linalg.generic {
      ...
      ins(%exp_scores0, %row_sums0 : tensor<7x2xf32>, tensor<7xf32>)
      outs(%prob0_empty : tensor<7x2xf32>) {
    ^bb0(%exp_score: f32, %row_sum: f32, %out: f32):
      %probability = arith.divf %exp_score, %row_sum : f32
      linalg.yield %probability : f32
    } -> tensor<7x2xf32>

  %prob1_empty = tensor.empty() : tensor<7x2xf32>
  %probabilities1 = linalg.generic {
      ...
      ins(%exp_scores1, %row_sums1 : tensor<7x2xf32>, tensor<7xf32>)
      outs(%prob1_empty : tensor<7x2xf32>) {
    ^bb0(%exp_score: f32, %row_sum: f32, %out: f32):
      %probability = arith.divf %exp_score, %row_sum : f32
      linalg.yield %probability : f32
    } -> tensor<7x2xf32>

  %value_head0_empty = tensor.empty() : tensor<2x64xf32>
  %value_head0 = linalg.generic {
      ...
      ins(%value_cache0 : tensor<2x64xf32>)
      outs(%value_head0_empty : tensor<2x64xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<2x64xf32>

  %value_head1_empty = tensor.empty() : tensor<2x64xf32>
  %value_head1 = linalg.generic {
      ...
      ins(%value_cache1 : tensor<2x64xf32>)
      outs(%value_head1_empty : tensor<2x64xf32>) {
    ^bb0(%value: f32, %out: f32):
      linalg.yield %value : f32
    } -> tensor<2x64xf32>
  ...
  %context_heads0 = linalg.matmul
      ins(%probabilities0, %value_head0 : tensor<7x2xf32>, tensor<2x64xf32>)
  ...
  %context_heads1 = linalg.matmul
      ins(%probabilities1, %value_head1 : tensor<7x2xf32>, tensor<2x64xf32>)
