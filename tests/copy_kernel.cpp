...
int main(void) {
  ...
  if (count == 0) {
    printf("copy_kernel: skipped, no GPU devices\n");
    lr_shutdown();
    return 0;
  }
  ...
  void *hsaco = NULL;
  size_t hsaco_size = 0;
  if (!read_file(LRRT_COPY_HSACO, &hsaco, &hsaco_size)) {
  ...
  status = lr_kernel_get(module, "copy_kernel", &kernel);
